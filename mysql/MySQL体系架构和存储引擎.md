...
如果表的类型是`InnoDB`，那么是15。因为InnoDB 表只是**把自增主键的最大ID记录到内存中**，所以重启数据库或对表进行OPTION操作，都会导致最大ID丢失。
...
> 在select count(?) from t 这样的查询语句中，count(*)、count(主键ID)、count(字段)和count(1)等不同用法的性能又那些区别？
