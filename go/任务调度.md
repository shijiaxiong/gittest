...
- 优先把goroutine放入P的本地队列，如果队列满了则通过runqputslow函数放入全局队列。
...
- 调用clone函数创建一个系统线程
- 创建完成后父线程继续自己的工作
- 子线程做一系类初始化工作，会从父线程的寄存器中获取到m结构体对象、g、m.g0的地址等



## Q:主动调度

- g的状态变更`_Grunning`->`——Grunnale`
- 接触工作线程m和g之间的关系
- 主动调度之后g去了全局队列
...
- _Pssyscall（系统调用状态的P）：gotoutine在内核执行系统调用。
