...
- 服务端的**I/O多路复用模块**监测到连接套接字变得可读时，会将事件通过文件事件分配器，分配给命令请求处理器，服务器将调用**命令请求处理器**来执行以下操作：
  - 将命令请求保存到客户端状态的输入缓冲区。
...
- 通过`CULSTER ADDSLOTS`命令将一个或多个节点指派给节点负责。
...
- 作用：`redis-cluster-proxy`是`redis-cluster`集群节点的代理。通过代理可以像单实例一样实现redis集群的访问。(类似分库分表中间件)
